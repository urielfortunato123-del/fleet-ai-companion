...
export interface FileAttachment {
  ...
  url?: string; // public URL for images
  extractedText?: string; // extracted content for excel/pdf
...
export function useChatFileUpload() {
  ...
  const extractExcelContent = (file: File): Promise<string> => {
      ...
      try {
        if (type === "image") {
          ...
          newAttachments.push({
            name: file.name,
            type: "pdf",
            extractedText: `[Arquivo PDF enviado: ${file.name}]`,
          });
        }
      } catch (err) {
